...
export default function Wheel() {
  const { prizes, updatePrize, audioSettings, addPrizeLog } = useWheel();
  ...
  const getWinnerByPercentage = () => {
    const availablePrizes = prizes
      .map((prize, index) => ({ prize, index }))
      .filter((p) => p.prize.stock > 0);

    if (availablePrizes.length === 0)
      return { prize: prizes[0], index: 0 };

    const totalPercentage = availablePrizes.reduce(
      (acc, p) => acc + p.prize.percentage,
      0
    );
    let random = Math.random() * totalPercentage;

    for (let i = 0; i < availablePrizes.length; i++) {
      if (random < availablePrizes[i].prize.percentage) {
        return availablePrizes[i];
      }
      random -= availablePrizes[i].prize.percentage;
    }
  ...
  const spin = () => {
    ...
    const { prize, index } = getWinnerByPercentage();
    ...
    setTimeout(() => {
      ...
      addPrizeLog(prize); // Log the prize
  ...
  return (
    <div className="relative flex flex-col items-center justify-center">
      ...
      <button
        onClick={spin}
        disabled={
          isSpinning || prizes.length === 0 || !prizes.some((p) => p.stock > 0)
        }
